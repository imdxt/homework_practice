...
int fb_line222(u16_t *buf32, fb_info fb_inf,int x1,int y1,int x2,int y2)
{
    int dx = x2-x1;
    int dy = y2-y1;
    int inc = ((dx*dy>0) ? 1:-1);
...
int fb_line2(const char *filename, fb_info fb_inf)
{
    ...
    for(i = 0;i < 40 ;i++)
    {
        for(j = 0;j < 20;j++)
        {
            ...
            if(i < 24)
                fb_line222(buf32, fb_inf,fb_inf.w,i + j*24,0,fb_inf.h - 1 - i - j*24);
            fb_line222(buf32, fb_inf,i + j*40, 0 ,fb_inf.w - i - j*40,fb_inf.h - 1);
        }
               usleep(10);
...
int display_scale1(char *filename, fb_info fb_inf,int jpeg_w,int jpeg_h)
{
    ...
    for(i =50,n = 0,m = 0; i < fb_inff.w+50; i = i + 1,n = n + 4,m++){
        for (j = 0; j < fb_inff.w; j++){
            if(*status_p == 0)goto biao;
                fb_line222(buf32, fb_inff,j ,fb_inf.h - 1,i,fb_inf.h);
        }
    }
