    ...
    for(i = 0; i < 24; i++){
        for (j = 0; j < 40; j++){
            for(m = 0; m < 20; m=m+2){
                ...
                fb_pixel(fb_inf, j + 40*m, i + 24*n, buf32[j+40*m + (i+24*n) * fb_inf.w]);
                fb_pixel(fb_inf, j + 40*(m+1), i + 24*(n+1), buf32[j+40*(m+1) + (i+24*(n+1)) * fb_inf.w]);
                fb_pixel(fb_inf, j + 40*m, fb_inf.h/20 - i - 1 + 24*(n+1), buf32[j+40*m + (fb_inf.h/20 - i - 1 +24*(n+1)) * fb_inf.w]);
                fb_pixel(fb_inf, j + 40*(m+1), fb_inf.h/20 - i - 1 + 24*n, buf32[j+40*(m+1) + (fb_inf.h/20 - i - 1 +24*n) * fb_inf.w]);
               }
            }
            //usleep(100);
        } 
...
int display_jpeg1(char *filename, fb_info fb_inf,int y,int z)
{
...
int display_scale(char *filename, fb_info fb_inf)
{
    ...
    for(m = fb_inf.w,n = fb_inf.h;m >= 0 && n >= 0; n = n - 24,m = m - 40)
    {
