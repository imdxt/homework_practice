    ...
    switch(*status_p)
    {
        case 0:
            {   
                ...
                {
                    display_string("浏览", 82, 296, fb_inf, PRE_COLOR);
                    tmp = 1;
                }
                else
                ...
                {
                    display_string("幻灯", 235, 339, fb_inf, PRE_COLOR);
                    tmp = 2;
                }
                else
                ...
                {
                    display_string("音乐", 368, 296, fb_inf, PRE_COLOR);
                    tmp = 3;
                }
                else
                ...
                {
                    display_string("我们", 533, 295, fb_inf, PRE_COLOR);
                    tmp = 4;
                }
                else
                ...
                {  
                    display_string("退出", 658, 215, fb_inf, PRE_COLOR);
                    tmp = 5;
                }
                else
        ...
        case 3:
            {
                ...
                //if(*play_flag == 0) goto music11;
                if(*song_disp_flag == 1)
                    display_song(mp3_num);
                if(*play_flag &&((x>JPG_UP_X1) && (y>JPG_UP_Y1) && (x<JPG_UP_X2) && (y<JPG_UP_Y2)))
                {
                    display_string("上一首", 143, 379, fb_inf, PRE_COLOR);
                    return 1;
                }
                else
                    display_string("上一首", 143, 379, fb_inf, COLOR);

                if(*play_flag&&((x>JPG_DOWN_X1) && (y>JPG_DOWN_Y1) && (x<JPG_DOWN_X2) && (y<JPG_DOWN_Y2)))
                {
                    display_string("下一首", 284, 385, fb_inf, PRE_COLOR);
                    return 2;
                }
                else
                    display_string("下一首", 284, 385, fb_inf, COLOR);
                //music11:
                if((x>PREVIEWEXIT_X1) && (y>PREVIEWEXIT_Y1) && (x<PREVIEWEXIT_X2) && (y<PREVIEWEXIT_Y2))
                {
                    display_string("返回", 690, 434, fb_inf, PRE_COLOR);
                    return 3;
                }
                else
                    display_string("返回", 690, 434, fb_inf, COLOR);

                if(((x>PREVIEWPLAY_X1) && (y>PREVIEWPLAY_Y1) && (x<PREVIEWPLAY_X2) && (y<PREVIEWPLAY_Y2)))
                {   display_string("播放", 178, 442, fb_inf, PRE_COLOR);
                    //if(*play_flag!=0)
                    return 4;
                }
                if(*play_flag == 0) 
                    ...
                    //if (*play_flag == 1) 
                       display_string("播放", 178, 442, fb_inf, COLOR);
                    //if (*play_flag == 0) 
                    //{
                       //display_string("播放", 178, 442, fb_inf, PRE_COLOR);
                    //}
                ...
                {
                    display_string("列表", 277, 331, fb_inf, PRE_COLOR);
                    return 5;
                }
                else
                    display_string("列表", 277, 331, fb_inf, COLOR);
            }
            return 0;
        case 1:
            {   
                ...
                if(mid_flag == 0)
                {
                    ...
                    {
                        display_string("上一页", 234, 450, fb_inf, PRE_COLOR);
                        return 1;
                    }
                    else
                        display_string("上一页", 234, 450, fb_inf, COLOR);

                    if(x>SCAN_DOWN_X1 && y>SCAN_DOWN_Y1 && x<SCAN_DOWN_X2 && y<SCAN_DOWN_Y2)
                    {
                        display_string("下一页", 574, 449, fb_inf, PRE_COLOR);
                        return 2;
                    }
                    else
                        display_string("下一页", 574, 449, fb_inf, COLOR);

                }
                if((x>SCAN_EXIT_X1) && (y>SCAN_EXIT_Y1) && (x<SCAN_EXIT_X2) && (y<SCAN_EXIT_Y2))
                {
                    display_string("返回", 706, 46, fb_inf, PRE_COLOR);
                    return 3;
                }
                else
                    display_string("返回", 706, 46, fb_inf, COLOR);
            }
...
int restoresong(fb_info fb_inf,int x,int y)
{
    ...
    for (j=0; j<song_h;j++)
    {
        for (i=0; i<song_w;i++)
        {
            color = *(u16_t*)(fb_inf.fbmem + ((y + j) * fb_inf.w + x + i) * fb_inf.bpp/8);
